...
import com.edge.printdemosunmi.model.TransactioModel
...
class SunmiPrinterHelper {
    ...
    fun initSunmiPrinterService(context: Context?): String {
        var result = "Success"
        try {
    ...
    fun printHeader() {
        ...
        sunmiPrinterService!!.setFontSize(34f, null)
        sunmiPrinterService!!.printText("Edgeworks Store\n", null)
    ...
    fun printBody() {
        ...
        sunmiPrinterService!!.setAlignment(LEFT, null)
        sunmiPrinterService!!.printText("Item1", null)
        sunmiPrinterService!!.setAlignment(RIGHT, null)
        sunmiPrinterService!!.printText("3.5\n", null)
        sunmiPrinterService!!.printText("Qty=2\n", null)
    ...
    fun printTest() {
        printHeader()
        sunmiPrinterService!!.lineWrap(3, null)
        var datas: List<TransactioModel> = emptyList()

        var data1 = TransactioModel()
        data1.Amount = 3.5f
        data1.Qty = 2f
        data1.ItemName = "Item 1"
        datas += data1

        data1 = TransactioModel()
        data1.Amount = 3.5f
        data1.Qty = 2f
        data1.ItemName = "Item 1"
        datas += data1
        ...
        var totalAmount = 0f
        var width: IntArray? = intArrayOf(2, 2)
        var align: IntArray? = intArrayOf(LEFT, RIGHT)

        for (data in datas) {
            //Print ItemName
            sunmiPrinterService!!.printText("Item1\n", null)

            //Print Qty & Amount
            val qty = data.Qty
            val amount = data.Amount
            totalAmount += amount
            var data: Array<String>? = arrayOf("Qty=$qty", "$amount")

            sunmiPrinterService!!.printColumnsString(data, width, align, null)
        }

        var dataAmount:Array<String>? = arrayOf("Total", "$totalAmount")
        sunmiPrinterService!!.printColumnsString(dataAmount, width, align, null)
